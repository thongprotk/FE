...
export const cardService = {
   ...
   */
  async getDueCards(deckId: string): Promise<PaginatedResponse<Card>> {
    const response = await apiClient.get<PaginatedResponse<Card>>(
  ...
  /**
   * Get quiz cards with multiple choice options
   */
  async getQuizCards(deckId: string): Promise<QuizItem[]> {
    const response = await apiClient.get<QuizItem[]>(
      `/decks/${deckId}/cards/quiz`,
    );
    return response.data;
  },
};
